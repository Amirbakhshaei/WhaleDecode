...
def test_builds_three_buttons_with_fee_params_for_base():
    ...
    for url in links.values():
        assert "#base/swap/ETH/0xToken" in url
        if "?" in url:
            assert "referrer=0xFEE" in url and "fee=80" in url


def test_arbitrum_slug_and_no_recipient_omits_fee():
    links = build_swap_links("arbitrum", "0xABC")
    assert all("#arbitrum/swap/" in u for u in links.values())
    assert all("referrer=" not in u and "fee=" not in u for u in links.values())
